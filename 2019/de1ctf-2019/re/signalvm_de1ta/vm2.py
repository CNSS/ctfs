...
caltors = ["+=", "-=", "*=", "/=", "%=", "|=", "&=", "^=", "<<=", ">>="] 
gotos = ["goto", "if (!flags) goto", "if (flags) goto", \
        "if (flags > 0) goto", "if (flags >= 0) goto", \
        "if (flags < 0) goto", "if (flags <= 0) goto"]
...
def decode(codes):
    ...
    ip, length = 0, len(codes)
    while ip < length - 4:
        size, im = 0, 0
        # 确认大小（特殊情况会具体更改，如 cmp ）
        if codes[ip + 1] == 1:
            size, im = 7, get_num(codes[ip + 3: ip + 7])
        else :
            im = codes[ip + 3]
            size = 4
        ...
        if codes[ip] == 0xcc: 
            if codes[ip + 1] == 1:
                cmds.append("_%d: regs[%d] = %d;" % (\
                    ip, codes[ip+2], im))
            elif codes[ip + 1] == 2:
                size = 4
                cmds.append("_%d: regs[%d] = zero[regs[%d]];" % (\
                    ip, codes[ip+2], im))
            elif codes[ip + 1] == 32:
                size = 4
                cmds.append("_%d: zero[regs[%d]] = regs[%d];" % (\
                    ip, codes[ip+2], im))
            elif codes[ip + 1] == 0:
                size = 4
                cmds.append("_%d: regs[%d] = regs[%d];" % (\
                    ip, codes[ip + 2], im))
        # goto
        elif codes[ip] == 0 and codes[ip + 1] == 0\
            and codes[ip + 2] >= 0 and codes[ip + 2] <= 6: 
            size, im = 7, get_num(codes[ip + 3: ip + 7])
            cmds.append( "_%d: %s _%d;" % (\
                ip, gotos[codes[ip+2]], ip + bv4(im)))
        
        # calculate
        elif codes[ip] == 6 and  \
            codes[ip + 1] <= 9 and codes[ip + 1] >= 0:
            if codes[ip + 2] == 1:
                size, im = 8, get_num(codes[ip + 4: ip + 8])
                cmds.append("_%d: regs[%d] %s %d;" % (\
                    ip, codes[ip + 3], caltors[codes[ip + 1]], im))
            else :
                size, im = 5, codes[ip + 4]  
                cmds.append("_%d: regs[%d] %s regs[%d];" % (\
                    ip, codes[ip + 3], caltors[codes[ip + 1]], im))
        # 有点没懂 为什么 是 ip + 10 / + 7
        # cmp
        elif codes[ip + 2] == 0xf6 and codes[ip + 3] == 0xf8:
            if codes[ip + 4] == 1:
                size, im = 10, get_num(codes[ip+6: ip+10])
                cmds.append("_%d: flags = regs[%d] - %d;" % (\
                    ip, codes[ip + 5], im))
            elif codes[ip + 4] == 0 :
                size, im = 7, codes[ip + 6]
                cmds.append("_%d: flags = regs[%d] - regs[%d];" % (\
                    ip, codes[ip + 5], im))
        # error
        else :
            print("error", sizelist,\
                binascii.hexlify(bytearray(codes[ip: ip + 10])))
            break
        ip += size
...
def enc():
    ...
    success = zero[1]
    while success == 48:
        i1, i2, off, ii, res = 101, 0, 0, 0, 0
        while i2 < 100:
            tmp = magic[(off + (ii * (ii + 1)/2 ))]
            out1[i2] = tmp
            res += tmp
            off += zero[i1] == 49
            i1, i2, ii = i1 - 1, i2 + 1, ii + 1
        if res > ans:
            ans = res
            out2 = out1
        i = 100
        while zero[1 + i] != 48 and i >= 0:
            zero[1 + i] ^= 1
            i -= 1
        zero[1 + i] = 49
        success = zero[1]
    flag = ''.join([chr(x) for x in out2])
    print(flag)

ccodes = "int regs[10] = {0}, flags = 0;\nchar zero[0x100] = {0};\n\nint vm()\n{\n"
...
if __name__ == "__main__":
    ...
    with open("vm.c", "w") as f:
        f.write(ccodes)
        f.close()
        os.system("gcc -c -o vm vm.c")
    with open("magic", "wb") as f:
        f.write(bytearray(magic))
    os.system("g++ -o sol sol.cpp && ./sol")
    # enc()